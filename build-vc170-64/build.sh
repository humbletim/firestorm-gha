...
autobuild print --json | jq '[.installables[]|{key:.name,value:{name,version,copyright,url:(.platforms|.windows64//.common//.windows).archive.url,hash:(.platforms|.windows64//.common//.windows).archive.hash}}]|from_entries' > _autobuild.json

# autobuild print --json | jq '.installables[].platforms|.windows64//.common//.windows|.archive.url' -r > autobuild.urls

packages="ogg_vorbis openal apr_suite boost expat xxhash zlib-ng jsoncpp xmlrpc-epi glh_linear \
