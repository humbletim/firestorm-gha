...
for x in packages CMakeFiles msvc copy_win_scripts sharedlibs
    llcommon newview/CMakeFiles/firestorm-bin.dir; do
...
function ht-ln() {
  ...
  local cmd="mklink $opts $(cygpath -w $linktname) $(cygpath -w $target)"
  echo "[ht-ln] $cmd"
...
if [[ -n "$GITHUB_ACTIONS" ]] ; then
    ...
    #export -f msvcdir
    grep msvc_dir build_vars.env >/dev/null || { echo "msvcdir=$(get_msvcdir)" | tee -a build_vars.env ; }

    ht-ln $build_dir/sharedlibs/Release $build_dir/sharedlibs
    ht-ln $(msvcdir) $build_dir/msvc
    ...
    pip install llsd # needed for viewer_manifest.py invocation
fi
...
function download_packages() {
    jq -r '.[]|.url' packages-info.json | grep http \
      | parallel --will-cite -j4 'echo {} >&2 && wget -nv -P $build_dir/autobuild-cache -N {}'
}
function verify_downloads() {
    jq -r '.[]|"name="+.name+" hash="+.hash+" url="+(.url//"null")' packages-info.json | grep -v url=null \
       | parallel --will-cite -j4 '{} ; echo $hash $build_dir/autobuild-cache/$(basename $url) | md5sum --quiet -c - '
}

function untar_packages() {
    jq -r '.[]|.url' build-vc170-64/packages-info.json | grep -vE '^null$' \
       | parallel --will-cite -j4 'basename {} ; tar -C "$packages_dir" -xf $build_dir/autobuild-cache/$(basename {})'
}
