function(boost_modular_build)
    ...
    if(VCPKG_CMAKE_SYSTEM_NAME STREQUAL "Linux" AND VCPKG_TARGET_ARCHITECTURE STREQUAL "arm64")
        set(BOOST_BUILD_PATH "${CURRENT_INSTALLED_DIR}/../x64-linux/tools/boost-build-160")
    elseif(CMAKE_HOST_WIN32 AND VCPKG_CMAKE_SYSTEM_NAME AND NOT VCPKG_CMAKE_SYSTEM_NAME STREQUAL "WindowsStore")
        get_filename_component(BOOST_BUILD_PATH "${CURRENT_INSTALLED_DIR}/../x86-windows/tools/boost-build-160" ABSOLUTE)
    elseif(NOT VCPKG_TARGET_ARCHITECTURE STREQUAL "x64" AND NOT VCPKG_TARGET_ARCHITECTURE STREQUAL "x86")
        get_filename_component(BOOST_BUILD_PATH "${CURRENT_INSTALLED_DIR}/../x86-windows/tools/boost-build-160" ABSOLUTE)
    else()
        set(BOOST_BUILD_PATH "${CURRENT_INSTALLED_DIR}/tools/boost-build-160")
    endif()

    if(NOT EXISTS "${BOOST_BUILD_PATH}")
        if(VCPKG_CMAKE_SYSTEM_NAME STREQUAL "Linux" AND VCPKG_TARGET_ARCHITECTURE STREQUAL "arm64")
            message(FATAL_ERROR "The x64 boost-build-160 tools must be installed to build arm64 for Linux. Please run `vcpkg install boost-build-160:x64-linux`.") 
        else()
            message(FATAL_ERROR "The x86 boost-build-160 tools must be installed to build for non-x86/x64 platforms. Please run `vcpkg install boost-build-160:x86-windows`.")
        endif()
    endif()
    ...
    set(_bm_DIR ${CURRENT_INSTALLED_DIR}/share/boost-build-160)

    if(NOT VCPKG_CMAKE_SYSTEM_NAME OR VCPKG_CMAKE_SYSTEM_NAME STREQUAL "WindowsStore")
        set(BOOST_LIB_PREFIX)
        set(BOOST_LIB_RELEASE_SUFFIX -vc140-mt.lib)
        set(BOOST_LIB_DEBUG_SUFFIX -vc140-mt-gd.lib)
        set(BOOST_LIB160_RELEASE_SUFFIX -vc140-mt-1_60.lib)
        set(BOOST_LIB160_DEBUG_SUFFIX -vc140-mt-gd-1_60.lib)
    else()
    ...
    if(VCPKG_CMAKE_SYSTEM_NAME AND NOT VCPKG_CMAKE_SYSTEM_NAME STREQUAL "WindowsStore")
        ...
        vcpkg_configure_cmake(
            SOURCE_PATH ${CURRENT_INSTALLED_DIR}/share/boost-build-160
            PREFER_NINJA
            OPTIONS
                "-DPORT=${PORT}"
                "-DFEATURES=${FEATURES}"
    ...
    foreach(LIB ${INSTALLED_LIBS})
        ...
        #string(REPLACE "-1_60" "" NEW_FILENAME ${NEW_FILENAME}) # To enable CMake > 3.10 to locate the binaries
